...
E_c = 21e10
rho_c = 48.62/A_c
X_1 = 4           # Also equal to l_2^*, the distance from the TMD to the lower end
l_c = 200
phi = pi/6
T_0 = 4.64e5
...
c = zeta/epsilon
X_1_star = X_1/l_c     # Also equal to l_2
...
def get_v(x):
    ...
    if x > 1 - X_1_star:
        return -1/l_c*(- 3*A*cos(phi)*X*g*rho*(A*((-1/36 + (X_1_star**2 - 4/3*X_1_star + 1/3)*c**2 +
    ...
    # Higher side of the cable
        ...
        return -1/l_c*(3*A*cos(phi)*rho*g*(L - X)*(A*(((X_1_star - 1/3)*c + X_1_star/2 - 1/3)*c*X_1_star*L**2 + 
